...
struct State{
    ...
    bool valid() const
    {
        return x>=0 && y>=0 && x < costs[0].size() && y < costs.size() && straight_counter <= 3;
    }

    std::string position() const
    {
        std::stringstream ss;
        ss << x << " " << y << " " << (int)direction;
        return ss.str();
    }
};

std::priority_queue<State, std::vector<State>, std::function<bool(const State&, const State&)>> check_queue;
std::unordered_map<std::string, int> min_straight;
...
void try_add_state(const State& state)
{
    if (state.valid())
    {
        ...
        if (min_straight[p] == 0 | state.straight_counter < min_straight[p])
        {
            check_queue.push(state);
            min_straight[p] = state.straight_counter;
        }
    }
}
...
void solve()
{
    ...
    while (!check_queue.empty())
    {
        ...
        if (current.y == costs.size()-1 && current.x == costs[0].size()-1) break;
        check_queue.pop();
